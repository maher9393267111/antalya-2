...
const   Header =({title, description, image, children, summaryText, cutout, styles}) => {
  return (
    ...
    className={`${styles} bg-cover bg-center text-center pt-[182px] pb-[154px] px-[5%] lg:pt-[180px] lg:pb-[220px] relative`}>
      ...
      <div className={`z-30 relative ${image === 'home' ? 'flex flex-col items-start justify-start max-w-[550px] lg:left-20 -top-14' : ''}`}>
        ...
        <motion.h1 
          ...
          whileInView="onscreen"
          viewport={{ once: true, amount: 0 }}
          className={`text-[33px] ${image === 'home' ? 'text-left' : ''}  arabic font-semibold text-[#ffffff] sm:text-[40px] md:text-[68px] leading-[1.1]`}
        >
          {title}
        </motion.h1>
        ...
        <motion.p 
          ...
          whileInView="onscreen"
          viewport={{ once: true, amount: 0 }}
          className={`text-[19px] ${image === 'home' ? '' : 'mx-auto'}  arabic mt-[30px] font-light text-[#ffffe6] sm:text-[20px] md:text-[21px] max-w-2xl `}
        >
          {description}
        </motion.p>
